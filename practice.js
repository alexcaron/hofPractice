...
var applyCoupon = function (groceries, coupon) {
  return _.map(groceries, function(item) {
    item.salePrice = '$' + Math.round(100 * (parseFloat(item.price.slice(1)) * (1 - coupon))) / 100;
    return item;
  });
};
...
var countMessagesPerUser = function(tweets) {
  return _.reduce(tweets, function(totals, tweet) {
    name = tweet.user;
    totals[name] = (totals[name]) ? totals[name] + 1 : 1;
    return totals;
  }, {});
};
...
var movieNight = function (movies, timeLimit) {
  return _.reduce(movies, function(enoughTime, movie) {
    return enoughTime ? true : movie.runtime < timeLimit;
  }, false);
};
